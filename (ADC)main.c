...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    ...
    {
        adc_ready = 1;  // Flag that data is ready
    }
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // Start ADC with DMA
  printf("\r\nADC DMA Example Started!\r\n");
   printf("Buffer Size: %d samples\r\n", ADC_BUFFER_SIZE);

   // Start ADC with DMA
   if (HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_value, ADC_BUFFER_SIZE) != HAL_OK)
   {
	  ...
	  if(adc_ready)
	      {
	          ...
	          // Calculate average of all samples for better stability
	          uint32_t sum = 0;
	          ...
	          printf("\r\n");
	          printf("Average: %lu, Voltage: %.2fV\r\n\r\n", average, voltage);





	          // Toggle LED based on voltage threshold
	          if(voltage > 2.0f)
	          {
	          ...
	          HAL_Delay(500); // Delay to avoid flooding UART
      }
  }
  /* USER CODE END 3 */
}
